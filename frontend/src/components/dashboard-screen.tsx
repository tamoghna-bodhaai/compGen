...
export function DashboardScreen() {
  ...
  const tracked = papers.filter((paper) => ["generating", "attention", "cancelled"].includes(paperDashboardState(paper)));
  ...
  async function controlGeneration(paper: PaperSummary, action: "pause" | "resume") {
    setControllingPaperId(paper.id);
    try {
      await api.post(`/papers/${paper.id}/generation/${action}`);
      await refresh();
      toast(action === "pause" ? "Generation paused. Completed questions are ready to review." : "Generation resumed.");

...
export function QuestionBankScreen() {
  ...
  const activeJob = ingestionJobs.find((job) => job.id === activeJobId) || null;
  useEffect(() => { if (activeJob?.state === "succeeded") void loadCatalog(); }, [activeJob?.state, loadCatalog]);
  ...
  async function ingest(event: React.FormEvent<HTMLFormElement>) {
    ...
    try { const result = await api.ingest(values); setIngestionJob(result.job); setActiveJobId(result.job.id); }

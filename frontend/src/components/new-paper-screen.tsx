...
export function NewPaperScreen() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const paper = await api.createPaper(view.payload);
      ...
      await refresh(); toast(`${view.payload.question_types.reduce((total, item) => total + item.count, 0)}-question generation queued. Track progress on the dashboard.`); router.push("/");

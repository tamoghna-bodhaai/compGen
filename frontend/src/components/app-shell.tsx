...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const routeName = pathname.startsWith("/papers/") ? "Paper editor" : pathname === "/new-paper" ? "Create paper" : pathname === "/question-bank" ? "Question bank" : "Dashboard";
  return <div className={s.shell}>
    ...
    <main className={s.workspace}>
      <header className={s.topbar}><div className={s.breadcrumb}><Icon name="document" /><span>Paper Studio</span><Icon name="chevron" /><strong>{routeName}</strong></div><Link href="/new-paper" className={s.topbarAction}><Icon name="plus" />New paper</Link></header>

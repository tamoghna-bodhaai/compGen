...
import type { BrandingConfig, Paper, PaperQuestion, SeedComparison, SeedQuestion } from "@/lib/types";
import { Dialog } from "@/components/dialog";
import { MathContent } from "@/components/math-content";
import { Button, Field, Input, Select, Textarea } from "@/components/ui";
import s from "@/styles/ui.module.css";

export type EditorDialog = "source" | "manual" | "branding" | "seed-comparison" | null;
...
export function PaperDialogs({ kind, paper, comparisonQuestion, onClose, onChanged, toast }: { kind: EditorDialog; paper: Paper; comparisonQuestion?: PaperQuestion | null; onClose: () => void; onChanged: () => Promise<void>; toast: (message: string, tone?: "default" | "error") => void }) {
  ...
  }, [kind, paper.id, paper.branding_config, comparisonQuestion?.id, sourceQuestions.length, toast]);
  ...
  const sharedTerms = generatedQuestion && selectedSeed ? sharedStemTerms(generatedQuestion.question_json.stem, selectedSeed.question_json.stem) : [];
  const comparisonRows = generatedQuestion && selectedSeed ? [
    ["Question type", generatedQuestion.question_type.replaceAll("_", " "), selectedSeed.question_type.replaceAll("_", " ")],
    ["Difficulty", `${generatedQuestion.difficulty}/5`, `${selectedSeed.difficulty}/5`],
    ["Primary concept", generatedQuestion.question_json.primary_concept || "Not recorded", selectedSeed.primary_concept || "Not recorded"],
    ["Options", `${generatedQuestion.question_json.options?.length || 0} options`, `${selectedSeed.question_json.options?.length || 0} options`],
    ["Answer", generatedQuestion.answer_json?.correct_answer || "Not recorded", selectedSeed.answer_json?.correct_answer || "Not recorded"],
  ] : [];
  return <>
    ...
    <Dialog open={kind === "seed-comparison"} title="Seed questions & comparison" subtitle="Review the current paper question against the seed questions used during generation." onClose={onClose} wide><div className={s.modalBody}>{comparisonError ? <p className={s.errorText} role="alert">{comparisonError}</p> : !comparison ? <p className={s.muted}>Loading referenced seed questions…</p> : !selectedSeed || !generatedQuestion ? <p className={s.muted}>No seed questions are currently available for this generated question.</p> : <div className={s.comparisonLayout}><aside className={s.seedSelector} aria-label="Referenced seed questions"><strong>Referenced seeds</strong>{comparison.seeds.map((seed, index) => <button type="button" key={seed.id} className={index === selectedSeedIndex ? s.seedSelected : ""} onClick={() => setSelectedSeedIndex(index)}>Seed {index + 1}<small>{seed.primary_concept || "Classified seed"}</small></button>)}</aside><section className={s.comparisonBody}><div className={s.comparisonMeta}><span className={s.chip}>Generation similarity {Number(comparison.generation_metadata.similarity_score || 0).toFixed(2)}</span><span className={s.chip}>Seed {selectedSeedIndex + 1} of {comparison.seeds.length}</span></div><div className={s.comparisonColumns}><article><h3>Current paper question</h3><MathContent as="div" className={s.detailStem} value={generatedQuestion.question_json.stem} />{!!generatedQuestion.question_json.options?.length && <ol className={s.optionList}>{generatedQuestion.question_json.options.map((option, index) => <li key={index}><MathContent value={option} /></li>)}</ol>}</article><article><h3>Referenced seed</h3><MathContent as="div" className={s.detailStem} value={selectedSeed.question_json.stem} />{!!selectedSeed.question_json.options?.length && <ol className={s.optionList}>{selectedSeed.question_json.options.map((option, index) => <li key={index}><MathContent value={option} /></li>)}</ol>}<p className={s.muted}>Source: {selectedSeed.source_reference || selectedSeed.source || "Not stated"}</p><p className={s.muted}>Archetype: {selectedSeed.question_archetype || "Not recorded"}</p></article></div><div className={s.comparisonFields}>{comparisonRows.map(([label, generated, seed]) => <div key={label}><span>{label}</span><strong>{generated}</strong><strong>{seed}</strong><em className={generated === seed ? s.comparisonSame : s.comparisonChanged}>{generated === seed ? "Same" : "Changed"}</em></div>)}</div><section className={s.overlapPanel}><strong>Surface wording overlap</strong><p>Shared normalized stem terms only; this is not a semantic-similarity judgment.</p><div className={s.chipRow}>{sharedTerms.length ? sharedTerms.map((term) => <mark key={term}>{term}</mark>) : <span className={s.muted}>No shared normalized terms</span>}</div></section>{comparison.missing_seed_question_ids.length > 0 && <p className={s.warningText}>{comparison.missing_seed_question_ids.length} recorded seed question{comparison.missing_seed_question_ids.length === 1 ? " is" : "s are"} no longer available in the bank.</p>}</section></div>}</div></Dialog>

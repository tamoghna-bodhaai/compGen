...
import type { Paper, PaperQuestion, QuestionDraft } from "@/lib/types";
...
export function PaperEditorScreen({ paperId }: { paperId: string }) {
  ...
  async function exportDocument(format: "pdf" | "docx") { if (!paper) return; try { await downloadPaper(paper, format, view === "answers" ? "answer_key" : "question_paper"); toast(`${format.toUpperCase()} download started.`); } catch (caught) { toast(caught instanceof Error ? caught.message : "Export failed.", "error"); } }
